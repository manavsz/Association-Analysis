...
def get_freq(dataset, candidates, min_support, verbose=False):
    ...
    support_data : dict
        The support data for all candidate itemsets.
    """
    support_data = dict.fromkeys(candidates,0.0)
    freq_list = []
    for t in dataset:
        for c in candidates:
            if c.issubset(t):
                count = support_data.get(c)
                count *= len(dataset)
                count += 1
                count = count/len(dataset)
                temp = {c:count}
                support_data.update(temp)
    for c in candidates:
        sup = support_data.get(c)
        if sup >= min_support:
            freq_list.append(c)
...
def apriori_gen(freq_sets, k):
    ...
    for a in freq_sets:
        for b in freq_sets:
            tempset = set(a.intersection(b))
            if len(tempset)==k-2:
                tempset.update(a)
                tempset.update(b)
                if tempset not in candidate_list:
                    candidate_list.append(frozenset(tempset))

    result = []
    for x in candidate_list:
        isfreq = True
        temp = list(combinations(x,k-1))
        for y in temp:
            newset = frozenset(y)
            if newset not in freq_sets:
                isfreq = False
        if isfreq and x not in result:
            result.append(x)
...
if __name__ == '__main__':
    ...
    python apriori_templete.py groceries.csv 0.02

    python apriori_templete.py groceries.csv 0.02 True
